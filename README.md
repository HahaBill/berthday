...
- CI script tests that verify D1 reuse/creation, reject IDs belonging to another database, and check deployment smoke-test responses including additional resources and import routes.
...
The resource and bulk-import update passed 180 unit tests, 45 Worker API tests, 19 migration/seed tests, 6 provisioning tests, and 8 smoke-script tests, plus typechecks and the production build. Browser checks covered the grid popup, dedicated/shared resources, preserving a filtered month and focusing the new row, mobile layouts, invalid workbooks, duplicate files, conflict creation, and pause/reload/resume. The production browser bundle parsed the full sample into 2,587 bookings and 635 vessels; reimporting skipped all 3,222 existing records without adding bookings or changing vessel facts. All temporary local QA data was removed.

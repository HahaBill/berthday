...
I make alternative berths and shifted dates deterministic. Best-fit berth ordering preserves large berths for large vessels, and the model cannot decide whether a booking is safe. The Ask bar only translates a question into filters the user can inspect and edit; a parser provides the same entry point when Workers AI is unavailable.

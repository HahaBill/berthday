...
export default function BookingForm({ draft, onClose }: { draft: BookingDraft; onClose: () => void }) {
  ...
  async function save(event: FormEvent) {
    ...
    try {
      await api<ReservationDTO>(draft.id ? `/reservations/${draft.id}` : '/reservations', { method: draft.id ? 'PATCH' : 'POST', body: JSON.stringify(payload) });
  ...
  return <Modal title={draft.id ? 'Edit booking' : 'New booking'} onClose={onClose} className="booking-dialog"><form onSubmit={save} className="booking-form">
    ...
    <div className="field-pair"><label className="field">Arrival date<input type="date" aria-label="Arrival date" value={startDate} required onChange={e => { setStartDate(e.target.value); if (e.target.value > endDate) setEndDate(e.target.value); }}/></label><label className="field">Last day at berth<input type="date" aria-label="Last day at berth" value={endDate} min={startDate} required onChange={e => setEndDate(e.target.value)}/><span className="field-hint">Inclusive — this day is occupied.</span></label></div>
    {berthStatus && <div className={`availability-hint ${berthStatus.status}`}><Status value={berthStatus.status}/><span>{berthStatus.status === 'busy' ? `Booked by ${berthStatus.conflicts.map(bookingName).join(', ')}` : berthStatus.status === 'too_short' ? `${vessel?.lengthFt} ft vessel; ${berth?.lengthFt} ft berth` : berthStatus.status === 'unverified' ? 'Enter a vessel length to verify fit.' : berthStatus.status === 'shared' ? 'Multiple bookings are permitted on this resource.' : 'This berth is free for the selected dates.'}</span></div>}

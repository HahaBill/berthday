...
import { parseFallback } from '../ask/fallback';
...
export const askRoutes = new Hono<WorkerEnv>().post('/', async c => {
  ...
  let intent: AskIntent | undefined, source: 'ai' | 'fallback' = 'fallback';
  ...
  if (c.env.AI) {
    ...
    try {
      ...
      const parsed = askIntentSchema.parse(normalizeModelOutput(typeof response === 'string' ? JSON.parse(response) : response));
  ...
  if (!intent) {
    ...
    try { vesselIds = await resolve(intent.vesselQuery); } catch {
      vesselIds = [];
      message = 'The vessel filter is ready, but vessel matches could not be loaded. Apply the filter or try again.';
    }
  }
  ...
  const { intent: action, ...filters } = intent;
